...
<div id="ContactBox_holder" runat="server">
    <div id="ContactBox_admin" runat="server" visible="false">
        <script type="text/javascript">
            //<![CDATA[
            
             function showAdminContactBox(SitId,PagId,ModId,Var)
            {
                var oWnd = $find("<%=RadWindow1_ContactBox.ClientID%>");
                var d = new Date();oWnd.SetUrl("http://<% Response.Write(Request.Url.Authority + Request.ApplicationPath); %>/Modules/Boxes/ContactBox/ContactBox_Admin.aspx?SitId=1&PagId=" + PagId + "&ModId=" + ModId + "&Var=" + Var + "&UID=" + d.getMilliseconds());
                oWnd.show();  
            }
