...
<div id="News_holder" runat="server" visible="false">
    <div id="News_admin" style="position: relative;" runat="server" visible="false">
        <script type="text/javascript">
            //<![CDATA[
            
            function showAdminNews(SitId,PagId,ModId)
            {
                var oWnd = $find("<%=RadWindow1_News.ClientID%>");
                var d = new Date();oWnd.SetUrl("http://<% Response.Write(Request.Url.Authority + Request.ApplicationPath); %>/Modules/Boxes/News/News_Admin.aspx?SitId=1&PagId=" + PagId + "&ModId=" + ModId + "&UID=" + d.getMilliseconds());
                oWnd.show();  
            }
    ...
    <%--                      <td style="background-color: #666666; text-align: center; height:25px; width:24px;">
              ...
              <td style="width:1px; font-size:1px;">&nbsp;</td>
                    <td style="background-color: #666666; text-align: center; height:25px; width:24px;">
                        <asp:ImageButton ID="imbAdminComment" onclick="img_AdminComment_Click" runat="server" /></td>--%>
                </tr>
                <tr>
